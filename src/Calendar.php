...
class Calendar
{
	...
	protected function createDateFromString($string, \DateTimeZone $tz = null) : ?\DateTime
	{
		...
		if (is_numeric($string)) {
			if (false !== ( $int = filter_var($string, FILTER_VALIDATE_INT) )) return $this->createDateFromInt($int, $tz);
			if (false !== ( $float = filter_var($string, FILTER_VALIDATE_FLOAT) )) return $this->createDateFromFloat($float, $tz);
		}
	...
	public function isDate($date, \DateTimeZone $tz = null, &$dt = null) : bool
	{
		...
		if (false !== ( $int = filter_var($date, FILTER_VALIDATE_INT) )) return (bool) $dt = $this->createDateFromInt($int, $tz);
		if (false !== ( $float = filter_var($date, FILTER_VALIDATE_FLOAT) )) return (bool) $dt = $this->createDateFromInt($float, $tz);
