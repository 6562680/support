...
Class Curl implements CurlInterface
{
	...
	{
		if (! $this->isCurl($curl)) {
			throw new InvalidArgumentException('Curl should be curl resource or closed one (created using this library)', func_get_args());
		}
	...
	public function opts(array $curls, bool $verbose = false) : array
	{
		...
		foreach ( $curls as $idx => $curl ) {
			if (! $this->isCurl($curl)) {
				throw new InvalidArgumentException('Curls should be array of curl resources or closed ones (created using this library)', func_get_args());
			}
