 ...
 * CurlFormatter
 */
class CurlFormatter implements CurlFormatterInterface
{
    ...
    protected function getInfoCodes() : array
    {
        return $this->infoCodes = $this->infoCodes
            ?? array_flip(static::getInfoNames());
    }
    ...
    public function detectInfoCode($info) : ?int
    {
        if (null !== $this->filter->filterInt($info)) {
            if (isset($this->getInfoCodes()[ $info ])) {
                return $info;
            }
        ...
        if (is_string($info)) {
            return static::getInfoNames()[ $info ] ?? false;
        }
    ...
    public function detectOptCode($opt) : ?int
    {
        if (null !== $this->filter->filterInt($opt)) {
            if (isset($this->getCurlOptCodes()[ $opt ])) {
                return $opt;
            }
        ...
        if (is_string($opt)) {
            return static::getCurlOptNames()[ $opt ] ?? false;
        }
     ...
     * @return array
     */
    protected static function getInfoNames() : array
    {
        return [
