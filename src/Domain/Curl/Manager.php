...
class Manager implements ManagerInterface
{
     ...
     * @return array
     */
    protected function getCurlOptCodes() : array
    {
        return $this->curlOptCodes = $this->curlOptCodes
            ?? array_flip(static::getCurlOptNames());
    }

    /**
     * @return array
     */
    protected function getInfoCodes() : array
    {
        return $this->infoCodes = $this->infoCodes
            ?? array_flip(static::getCurlInfoNames());
    }
    ...
    public function mergeOptions(array ...$curlOptArrays) : array
    {
        ...
        foreach ( $kwargs as $opt => $val ) {
            if (null === ( $optCode = $this->curlOptCodeVal($opt) )) {
    ...
    public function curlInfoCodeVal($curlInfoCode) : ?int
    {
        ...
        if (is_string($curlInfoCode)) {
            ...
            $curlInfoCodes = $this->getInfoCodes();
    ...
    {
        if (null === ( $val = $this->curlInfoCodeVal($curlInfoCode) )) {
            throw new InvalidArgumentException(
                [ 'Invalid CurlInfoCode passed: %s', $curlInfoCode ]
            );
        }
    ...
    {
        if (null === ( $val = $this->curlOptCodeVal($curlOptCode) )) {
            throw new InvalidArgumentException(
                [ 'Invalid CurlOptCode passed: %s', $curlOptCode ]
            );
        }
    ...
    {
        $keys = array_intersect_key($this->getCurlOptCodes(), $curlOptArray);
     ...
     * @return array
     */
    protected static function getCurlInfoNames() : array
    {
        return [
