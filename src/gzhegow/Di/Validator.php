...
class Validator
{
	...
	{
		if (! $this->isBind($bind)) {
			throw new InvalidArgumentException('Bind should be \Closure, class name or interface name', func_get_args());
		}
	...
	public function isDelegateClassOrFail($delegateClass)
	{
		if (! $this->isDelegateClass($delegateClass)) {
			throw new InvalidArgumentException('DelegateClass should be string or object that implements ' . DelegateInterface::class, func_get_args());
		}
	...
	public function isProvider($provider) : bool
	{
		return $this->isProviderObject($provider)
			|| $this->isProviderString($provider);
	}
	...
	public function isProviderOrFail($provider)
	{
		if (! $this->isProvider($provider)) {
			throw new InvalidArgumentException('Provider should be string or object that implements ' . ProviderInterface::class, func_get_args());
		}
	...
	public function isProviderObjectOrFail($provider)
	{
		if (! $this->isProviderObject($provider)) {
			throw new InvalidArgumentException('Provider should be object that implements ' . ProviderInterface::class, func_get_args());
		}
	 ...
	 * @return bool
	 */
	public function isProviderString($provider) : bool
	{
		return is_string($provider) && is_a($provider, ProviderInterface::class);
	}
	 ...
	 * @return static
	 */
	public function isProviderStringOrFail($provider)
	{
		if (! $this->isProviderString($provider)) {
			throw new InvalidArgumentException('Provider should be string that implements ' . ProviderInterface::class, func_get_args());
		}
